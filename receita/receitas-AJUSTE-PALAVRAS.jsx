		...
		if (modelo == "simples") {
			...
			for (var i = 0; i < receita.preparo.length; i++) {
				...
				for (var j = 0; j < etapa.passos.length; j++) {
					...
					var sTexto = textoPassoAPasso.textItem.contents;

					function escreveTexto() {
						var textoHeight = 0;
						if(sTexto.length>200) {
							textoPassoAPasso.textItem.contents = sTexto.substring(0,sTexto.indexOf(" ", 200));

							sTexto = sTexto.substring(sTexto.indexOf(" ", 200));
							var aTexto = sTexto.split(" ");
						
							palavrasOld = "";
							var j = 0;
							while (textoHeight < 580 || j == (aTexto.length-1)) {
								var palavra = aTexto[j];
								palavrasOld = textoPassoAPasso.textItem.contents;

								textoPassoAPasso.textItem.contents += palavra + " ";

								textoHeight = getRealTextLayerDimensions(textoPassoAPasso).height;
								if (textoHeight > 580) {
									sTexto = "";
									for (var n = 0; n < (aTexto.length - j); n++) {
										sTexto += aTexto[n+j];
										if(n < (aTexto.length - j)) {
											sTexto += " ";
										}
									}

									textoPassoAPasso.textItem.contents = palavrasOld;
									nPagina++;
							...
							textoPassoAPasso.textItem.contents = "";
							textoPassoAPasso.textItem.contents = sTexto;
							sTexto = "";
						}
